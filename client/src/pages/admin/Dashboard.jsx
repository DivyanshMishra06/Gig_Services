...
export default function AdminDashboard() {
  ...
  const pieData = [
    { name: t('adminDashboard.verified'), value: s.verifiedWorkers, color: '#0B8F4D' },
    { name: t('adminDashboard.pending'), value: s.totalWorkers - s.verifiedWorkers, color: '#F5823A' }
  ];
    ...
    <div style={{ padding: '32px 24px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div className="grid-4" style={{ marginBottom: '32px' }}>
        <div className="stat-card">
          ...
          <div className="stat-label">{t('adminDashboard.totalWorkers')}</div>
          <div className="stat-change up">↑ {s.cooperativeGrowth}% {t('adminDashboard.growth')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">👥</div>
          <div className="stat-value">{s.totalCustomers.toLocaleString()}</div>
          <div className="stat-label">{t('adminDashboard.totalCustomers')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">📋</div>
          <div className="stat-value">{s.todayBookings}</div>
          <div className="stat-label">{t('adminDashboard.todaysBookings')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">💰</div>
      ...
      <div className="grid-4" style={{ marginBottom: '32px' }}>
        <div className="stat-card">
          ...
          <div className="stat-label">{t('adminDashboard.verifiedWorkers')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">🟢</div>
          <div className="stat-value">{s.activeWorkers}</div>
          <div className="stat-label">{t('adminDashboard.activeNow')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">⏳</div>
          <div className="stat-value" style={{ color: 'var(--warning)' }}>{s.pendingVerifications}</div>
          <div className="stat-label">{t('adminDashboard.pendingVerifications')}</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">🏦</div>
          <div className="stat-value" style={{ color: 'var(--success)' }}>₹{(s.welfareFund / 100000).toFixed(1)}L</div>
          <div className="stat-label">{t('adminDashboard.welfareFund')}</div>
        </div>
      </div>
      ...
      <div className="grid-2" style={{ marginBottom: '32px' }}>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={revenueData}>
              ...
              <Tooltip
                ...
                formatter={(value) => [`₹${value.toLocaleString()}`, t('adminDashboard.revenue')]}
              />
        ...
        {/* Verification Pie */}
        <div className="chart-container">
          <h3>{t('adminDashboard.verificationStatus')}</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
      ...
      {/* Cooperatives */}
      <div className="card">
        <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '20px' }}>{t('adminDashboard.registeredCooperatives')}</h3>
        {cooperatives.length > 0 ? (
          <div style={{ overflowX: 'auto' }}>
            <table className="data-table">
              <thead>
                <tr>
                  <th>{t('adminDashboard.name')}</th><th>{t('adminDashboard.location')}</th><th>{t('adminDashboard.members')}</th><th>{t('adminDashboard.active')}</th><th>{t('adminDashboard.rating')}</th><th>{t('adminDashboard.welfareFund')}</th>
                </tr>
              </thead>
              <tbody>
                {cooperatives.map(c => (
          ...
          <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-secondary)' }}>
            <p>{t('adminDashboard.noCooperatives')}</p>
          </div>
        )}
      </div>
    </div>
